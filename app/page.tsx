...
function getPriorityColor(priority: string) {
  const colors: Record<string, string> = { critical: '#dc2626', high: '#ea580c', medium: '#2563eb', low: '#059669' };
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (studentError) { console.error('Failed to load student profile:', studentError); setLoading(false); return; }
  ...
  return (
    <main className="container">
      ...
      <div className="card" style={{ background: '#0f172a', color: 'white' }}>
        ...
        {maths && <div style={{ marginTop: '0.5rem' }}><p style={{ color: '#cbd5e1', fontSize: '0.875rem' }}><strong>Maths:</strong> {maths.subjects_catalog?.name} — Level {getLevel(maths.current_percentage)} ({maths.current_percentage}%)</p></div>}
      </div>

      {profile?.career_pathway === 'university' && <div className="card" style={{ background: '#eff6ff', border: '1px solid #bfdbfe' }}><div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}><AppIcon name="graduation" size={20} /><h2 style={{ color: '#1e40af', fontSize: '1.1rem', marginBottom: 0 }}>University Admission Requirements</h2></div><div style={{ marginTop: '0.75rem', fontSize: '0.875rem', color: '#334155', lineHeight: 1.7 }}><p><strong>Bachelor Pass:</strong> APS 23+ • English Level 4 (50%+) • LO Level 3 (40%+)</p><p><strong>UJ BA:</strong> APS 27 • English Level 5 (60%+)</p><p><strong>UP BA:</strong> APS 28 • English Level 5 (60%+)</p><p><strong>Wits BA:</strong> APS 36+ • English Level 5 (60%+)</p></div></div>}
