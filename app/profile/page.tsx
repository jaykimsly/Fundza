...
export default function ProfilePage() {
    ...
    const load = async () => {
      try {
        ...
        if (identity) {
          ...
          const paths = [identity.photo_one_path, identity.photo_two_path];
  ...
  return (
    <main className="container fd-profile-page">
      ...
      <section className="fd-xp-panel fd-panel">
        <div className="fd-panel-heading"><div><span className="fd-eyebrow">LEVEL UP PROGRESSION</span><h2>Level {stats.level}</h2></div><strong>{stats.xp} / {stats.nextXp} XP</strong></div>
        <div className="fd-xp-track"><span style={{ width: `${Math.min(100, Math.round((stats.xp / stats.nextXp) * 100))}%` }} /></div>
        <p>{Math.max(0, stats.nextXp - stats.xp)} XP until your next level.</p>
      </section>

      <div className="fd-dashboard-grid">
        <section className="fd-panel">
          <div className="fd-panel-heading"><div><span className="fd-eyebrow">DAILY QUESTS</span><h2>Today's missions</h2></div><span className="fd-quest-count">{completedQuests}/{quests.length}</span></div>
          <div className="fd-quest-list">
            {quests.map(quest => (
              <Link key={quest.label} href={quest.href} className={`fd-quest ${quest.done ? 'is-done' : ''}`}>
                <span className="fd-quest-icon">{quest.done ? '✓' : '○'}</span>
                <span><strong>{quest.label}</strong><small>+{quest.xp} XP</small></span>
                <span className="fd-quest-arrow">→</span>
              </Link>
            ))}
          </div>
        </section>
        ...
        <section className="fd-panel">
          <div className="fd-panel-heading"><div><span className="fd-eyebrow">CHARACTER STATS</span><h2>Study attributes</h2></div></div>
      ...
      <section className="fd-panel fd-subject-panel">
        <div className="fd-panel-heading"><div><span className="fd-eyebrow">SKILL TREE</span><h2>Your subjects</h2></div><Link href="/profile/edit">Manage →</Link></div>
        <div className="fd-subject-grid">
          {subjects.map(subject => {
            ...
            return <div className="fd-subject-card" key={subject.id}><div><strong>{subject.subjects_catalog?.name || 'Subject'}</strong><span>{current}% → {target}% target</span></div><div className="fd-mini-track"><span style={{ width: `${progress}%` }} /></div></div>;
          })}
        </div>
      </section>

      <section className="fd-dashboard-grid">
        <section className="fd-panel fd-goal-card">
          <div className="fd-panel-heading"><div><span className="fd-eyebrow">MAIN QUEST</span><h2>{student.target_degree || 'Academic target'}</h2></div></div>
          <p>Target institution: <strong>{student.target_university || 'Not set'}</strong></p>
          <div className="fd-goal-meter"><span style={{ width: `${stats.mastery}%` }} /></div>
          <strong>{stats.mastery}% quest completion</strong>
        </section>
        <section className="fd-panel fd-identity-card">
          <div className="fd-panel-heading"><div><span className="fd-eyebrow">IDENTITY LOADOUT</span><h2>Your photos</h2></div><Link href="/setup">Manage →</Link></div>
          <div className="fd-photo-slots">
            {[photoUrls[0], photoUrls[1]].map((url, index) => <div className="fd-photo-slot" key={index}>{url ? <img src={url} alt={`Identity photo ${index + 1}`} /> : <span>{index + 1}</span>}</div>)}
          </div>
          <p>{identityPhotos ? 'Identity photos are connected to your learner account.' : 'Add your two identity photos to complete your profile loadout.'}</p>
        </section>
      </section>

      <section className="fd-panel fd-achievements">
        <div className="fd-panel-heading"><div><span className="fd-eyebrow">ACHIEVEMENT TROPHIES</span><h2>Milestones</h2></div></div>
        <div className="fd-trophy-grid">
          <div className={`fd-trophy ${subjects.length >= 6 ? 'unlocked' : ''}`}><span>◆</span><strong>Ready Player</strong><small>6+ subjects selected</small></div>
          <div className={`fd-trophy ${stats.currentAvg >= 60 ? 'unlocked' : ''}`}><span>✦</span><strong>Knowledge Seeker</strong><small>60% average reached</small></div>
          <div className={`fd-trophy ${streak >= 7 ? 'unlocked' : ''}`}><span>◇</span><strong>Consistent</strong><small>7 day streak</small></div>
          <div className={`fd-trophy ${legalComplete ? 'unlocked' : ''}`}><span>⬡</span><strong>Account Ready</strong><small>Legal access complete</small></div>
        </div>
      </section>

      <section className="fd-quick-links">
        <Link href="/study">Study Arena <span>→</span></Link>
        <Link href="/quiz">Quiz Battles <span>→</span></Link>
        <Link href="/exams">Past Paper Dungeon <span>→</span></Link>
        <Link href="/progress">Progress Map <span>→</span></Link>
      </section>

      <section className="fd-panel fd-legal-panel">
        <div className="fd-panel-heading"><div><span className="fd-eyebrow">ACCOUNT STATUS</span><h2>Access & legal</h2></div><span className={legalComplete ? 'fd-status-good' : 'fd-status-warn'}>{legalComplete ? 'READY' : 'ACTION REQUIRED'}</span></div>
