...
function geometry(kind: ProfileMediaKind, image: HTMLImageElement, zoom: number) {
  const cropHeight = CROP_WIDTH / ASPECT[kind];
...
export default function ProfileMediaUploader({ studentId, initialAvatarUrl, initialBackgroundUrl }: Props) {
  ...
  const onFile = async (file?: File) => {
    ...
    try {
      ...
      setCrop({ kind, objectUrl, image, zoom: 1, x: 0, y: 0 });
    } catch (err) { console.error(err); setError('That image could not be opened. Try another file.'); }
  };
