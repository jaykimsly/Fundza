...
type LiveQuestion = { id: string; question_text: string; question_type: string | null; difficulty: string | null; topic_id: string | null; question_options?: { label: string; option_text: string }[]; question_answers?: { correct_answer: string; explanation: string | null; steps: any } | null };

function toQuestion(q: LiveQuestion): Question {
  const answer = q.question_answers;
  const options = (q.question_options || []).sort((a,b) => a.label.localeCompare(b.label));
  return { id:q.id, questionText:q.question_text, questionType:q.question_type || 'multiple-choice', difficulty:q.difficulty || 'medium', topicId:q.topic_id || '', options:options.map(o => `${o.label}. ${o.option_text}`), correctAnswer:answer?.correct_answer || '', explanation:answer?.explanation || 'Review the worked answer and try the question again.', steps:Array.isArray(answer?.steps) ? answer.steps.map(String) : [] } as Question;
}
...
export default function Quiz({ topicId }: Props) {
  ...
  const loadQuestions = async () => {
    ...
    try {
      const query = supabase.from('questions').select('id, question_text, question_type, difficulty, topic_id, question_options(label, option_text), question_answers(correct_answer, explanation, steps)').limit(20);
      const { data, error } = topicId ? await query.eq('topic_id', topicId) : await query;
      if (error) throw error;
      const live = ((data || []) as LiveQuestion[]).map(toQuestion).filter(q => q.correctAnswer);
      const selected = live.length ? live : (topicId ? fallbackQuestions.filter(q => q.topicId === topicId) : fallbackQuestions.slice(0,5));
      setQuestions(selected.length ? selected : fallbackQuestions.slice(0,3));
      setCurrentIndex(0); setSelectedAnswer(null); setSubmitted(false); setScore(0); setShowResults(false); setRetryMode(false); setWrongQuestions([]);
    } catch (err: any) {
      console.error('Quiz load error:', err);
      const fallback = topicId ? fallbackQuestions.filter(q => q.topicId === topicId) : fallbackQuestions.slice(0,5);
      if (fallback.length) { setQuestions(fallback); setLoadError('Live question bank is unavailable, so Fundza is showing the available practice questions.'); }
      else setLoadError(err?.message || 'We could not load questions.');
  ...
  const handleRestart = () => { loadQuestions(); };

  const saveAttempt = async () => {
    const studentId = localStorage.getItem('fundza_student_id');
    if (!studentId) return;
    try {
      const { data: attempt } = await supabase.from('quiz_attempts').insert({ student_id:studentId, topic_id:topicId, score, total_questions:questions.length }).select('id').single();
      if (attempt?.id) await supabase.from('quiz_answers').insert(questions.map((q, i) => ({ attempt_id:attempt.id, question_id:q.id, selected_answer:i === currentIndex ? selectedAnswer : null, is_correct:i === currentIndex ? selectedAnswer === q.correctAnswer : false })));
    } catch (err) { console.error('Could not save quiz attempt:', err); }
  };
  ...
  if (showResults) {
    ...
    return <div className="card"><h2>{retryMode ? 'Retry Results' : 'Quiz Complete'}</h2>{loadError && <div className="warning-box">{loadError}</div>}<div style={{textAlign:'center',padding:'2rem 0'}}><div style={{fontSize:'3rem',fontWeight:700}}>{percentage}%</div><p style={{color:'#64748b',marginTop:'.5rem'}}>{score} correct out of {questions.length}</p></div>{!retryMode && wrongQuestions.length > 0 && <div style={{marginBottom:'1.5rem'}}><p style={{color:'#991b1b',marginBottom:'.75rem'}}>You got {wrongQuestions.length} question{wrongQuestions.length > 1 ? 's' : ''} wrong. Let's fix that.</p><button onClick={handleRetry} className="btn btn-secondary">Retry Wrong Questions</button></div>}<div style={{display:'flex',gap:'.75rem',flexWrap:'wrap'}}><button onClick={handleRestart} className="btn">Restart Quiz</button><Link href="/study" className="btn btn-secondary">Back to Study</Link><Link href="/progress" className="btn btn-success">View Progress</Link></div></div>;
  }
