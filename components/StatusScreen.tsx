...
const content: Record<StatusKind, { title: string; description: string; detail: string }> = {
  ...
  offline: {
    ...
    detail: 'Check Wi-Fi or mobile data. Fundza needs a connection for live lessons, AI, login and saved progress.',
  },
  '404': {
  ...
  ai: {
    title: 'Fundza AI could not help this time',
    description: 'The AI tutor could not create an answer right now.',
    detail: 'This can happen when the AI service is busy, your connection drops, or the question needs another attempt.',
  },
};
  ...
  return (
    <main className="status-page" role={kind === 'error' || kind === 'ai' ? 'alert' : 'main'}>
      <div className="status-card">
        <div className="status-icon" aria-hidden="true">{kind === 'loading' ? '◌' : kind === 'offline' ? '⌁' : kind === '404' ? '404' : kind === '503' ? '503' : kind === 'ai' ? 'AI' : '!'}</div>
        ...
        {kind === 'ai' && <div className="status-tip"><strong>What you can do:</strong><br />Check your connection, simplify the question, then try again. Fundza will not pretend an AI answer worked when it did not.</div>}
