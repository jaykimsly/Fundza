...
// Kept as a compatibility export for existing Fundza routes.
...
export async function generateUploadedFileJson<T>(
  ...
  // Deliberately do not route PDFs to a paid model. The current free Gateway
  // default is multimodal for images, but does not advertise PDF input.
...
export async function embedText(text: string, title?: string) {
  ...
  const result = await ai.models.embedContent({
    ...
    contents: text,
    config: { taskType: 'RETRIEVAL_DOCUMENT', title, outputDimensionality: 768, autoTruncate: true } as any,
  });
...
export async function embedQuery(text: string) {
  ...
  const result = await ai.models.embedContent({
    model: GEMINI_EMBEDDING_MODEL,
    contents: text,
    config: { taskType: 'RETRIEVAL_QUERY', outputDimensionality: 768, autoTruncate: true } as any,
  });
  const values = result.embeddings?.[0]?.values;
