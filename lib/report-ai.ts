...
const GEMINI_REPORT_TIMEOUT_MS = 120000;

type GeminiError = { status?: number | string; message?: string; code?: string | number };

export function isGeminiReportConfigured() {
  return apiKey.length > 20 && !apiKey.toLowerCase().includes('your-gemini');
}

function client() {
  if (!isGeminiReportConfigured()) throw new Error('GEMINI_API_KEY is not configured');
  return new GoogleGenAI({ apiKey, httpOptions: { apiVersion: 'v1', timeout: GEMINI_REPORT_TIMEOUT_MS } });
}
...
export async function generateGeminiJson<T>(input: string, schema: Record<string, unknown>): Promise<T> {
  ...
  return withModelFallback(async model => {
    const response = await ai.models.generateContent({
      model,
      contents: input,
      config: { responseMimeType: 'application/json', responseSchema: schema as any, thinkingConfig: { thinkingLevel: ThinkingLevel.MEDIUM } },
    });
    return parseJsonResponse<T>(response.text || '');
  });
}

export async function generateGeminiMultimodalJson<T>(prompt: string, fileData: string, mimeType: string, schema: Record<string, unknown>): Promise<T> {
  if (!mimeType.startsWith('image/') && mimeType !== 'application/pdf') throw new Error('UNSUPPORTED_FILE_TYPE');
  const ai = client();
  return withModelFallback(async model => {
    const response = await ai.models.generateContent({
      model,
      contents: [{ text: prompt }, { inlineData: { data: fileData, mimeType } }],
      config: { responseMimeType: 'application/json', responseSchema: schema as any, thinkingConfig: { thinkingLevel: ThinkingLevel.MEDIUM } },
    });
    return parseJsonResponse<T>(response.text || '');
  });
